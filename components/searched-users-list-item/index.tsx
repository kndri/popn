...
const SearchedUserListItem = (props: SearchedUserListItemProps) => {
	const { user } = props;
	const [userAvatarImageURL, setUserAvatarImageURL] = React.useState(
		user.avatarImageURL
	);
	...
	const loadUserImages = async () => {
		if (
			...
			setUserAvatarImageURL(
				user.avatarImageURL.substring(
					0,
					user.avatarImageURL.indexOf('.jpeg') + '.jpeg'.length
				)
			);
		}
	};
	...
	return (
		<TouchableOpacity
			onPress={() => {
				navigation.navigate('UserProfile', user.id);
			}}
		>
			<View style={styles.container}>
				<View style={styles.lefContainer}>
					<Image
						source={{ uri: `${userAvatarImageURL}` }}
						style={styles.avatar}
					/>
